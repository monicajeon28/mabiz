...
export async function GET(
  ...
  try {
    ...
    if (product.itineraryPattern && Array.isArray(product.itineraryPattern)) {
      ...
      (product.itineraryPattern as Array<{ country?: string; location?: string }>).forEach((day) => {
    ...
    const startDate = trip.startDate
      ...
      : (product.startDate ? new Date(product.startDate).toISOString().split('T')[0] : '');
    const endDate = trip.endDate
      ? trip.endDate.toISOString().split('T')[0]
      : (product.endDate ? new Date(product.endDate).toISOString().split('T')[0] : '');
    ...
    logger.error('[Purchase Customer Trip Info] Error:', error);
    return NextResponse.json(

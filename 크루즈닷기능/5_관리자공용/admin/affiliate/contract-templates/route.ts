...
import { DEFAULT_CONTRACT_TEMPLATES, ContractTemplate } from '@/lib/affiliate/contract-templates-data';
...
async function getCustomContractTypes(): Promise<string[]> {
  try {
    ...
    console.error('[Contract Templates] Failed to load custom types:', e);
  }
  return [];
}
  ...
  try {
    ...
    configs.forEach((config) => {
      ...
      try {
        ...
        console.error(`[Contract Templates] Failed to parse template for ${contractType}:`, e);
      }
    });
...
export async function POST(req: NextRequest) {
  try {
    ...
    logger.log(`[Contract Templates] Template ${isNew ? 'created' : 'saved'} for ${contractType} by admin ${sessionUser.id}`);
    ...
    return NextResponse.json(
      { ok: false, message: error?.message || '템플릿 저장 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (isCustom && (permanent || !isDefault)) {
      ...
      logger.log(`[Contract Templates] Custom template deleted: ${contractType} by admin ${sessionUser.id}`);
    ...
    logger.log(`[Contract Templates] Template reset for ${contractType} by admin ${sessionUser.id}`);
    ...
    return NextResponse.json(
      { ok: false, message: error?.message || '템플릿 삭제 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}

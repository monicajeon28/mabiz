...
async function executeSyncOperation(): Promise<SyncResponse> {
  ...
  try {
    ...
    logger.log(`[Batch1Sync] Starting Cloudinary upload for ${BATCH_LABEL}`);

    const localFiles = await getLocalImageFiles();
    const totalImages = localFiles.length;

    logger.log(`[Batch1Sync] Upload target: ${totalImages.toLocaleString()} images`);
    ...
    for (let i = 0; i < totalImages; i += CONCURRENT) {
      ...
      logger.log(`[Batch1Sync] ${progressMsg}`);
    ...
    logger.log('[Batch1Sync] Upload complete', {

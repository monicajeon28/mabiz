...
const IMAGES_BASE_PATH = '/home/userhyeseon28/projects/cruise-guide-app/public/local-assets/Images/Images';
const BATCH_4_OFFSET = 2640;
const BATCH_4_LIMIT = 2000; // 모든 나머지 이미지 처리
const PARALLEL_UPLOADS = 10;

/**
 * 배치 4/4: Images 폴더 이미지 Cloudinary 업로드 (나머지)
 *
 * 처리:
 * 1. ImageCache에서 cloudinaryUrl IS NULL인 레코드 중 배치 4 (2640+) 조회
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!cronSecret || authHeader !== `Bearer ${cronSecret}`) {
      logger.warn('[UploadImagesBatch4] 인증 실패 - 무단 접근 시도');
    ...
    logger.log('[UploadImagesBatch4] 배치 4/4 시작', {
      timestamp: new Date().toISOString(),
      offset: BATCH_4_OFFSET,
      limit: BATCH_4_LIMIT,
      parallelUploads: PARALLEL_UPLOADS,
    });

    // 1. ImageCache에서 cloudinaryUrl IS NULL인 레코드 중 배치 4 (offset: 2640, limit: 2000) 조회
    ...
    logger.log('[UploadImagesBatch4] 업로드 대상 조회 완료', {
    ...
    if (imagesToSync.length === 0) {
      logger.log('[UploadImagesBatch4] 업로드할 이미지 없음 - 작업 종료');
      return NextResponse.json({
        ok: true,
        uploaded: 0,
        failed: 0,
    ...
    for (let batchIndex = 0; batchIndex < batches; batchIndex++) {
      ...
      logger.log(
        `[UploadImagesBatch4] 배치 4/4: ${end}/${imagesToSync.length} (${progressPercent}%) 업로드 중...`
      );
      ...
      const results = await Promise.allSettled(
        batch.map(async image => {
          try {
            ...
            logger.log('[UploadImagesBatch4] 성공:', {
              imageId: image.id,
              fileName: image.fileName,
            ...
            logger.error('[UploadImagesBatch4] 실패:', {
              fileName: image.fileName,
              folder: image.folder,
              error: errorMessage,
            });
    ...
    // 배치 4/4 완료 로그
    logger.log('[UploadImagesBatch4] 배치 4/4 완료', {
      batchNumber: '4/4',
    ...
    return NextResponse.json({
      ...
      successRate: `${successRate}%`,
      elapsedTime: `${elapsedTime}초`,
      message: `배치 4/4 완료: ${uploaded}/${imagesToSync.length} 업로드, ${failed}개 실패`,
    ...
    logger.error('[UploadImagesBatch4] 예상치 못한 에러', {
      error: errorMessage,
      elapsedTime: `${elapsedTime}초`,

...
const BATCH_SIZE = 29; // 두 번째 배치 29개 (남은 모든 이미지)
const PARALLEL_UPLOADS = 10; // 동시 업로드 10개

/**
 * 배치 2/2: Images 폴더 이미지 Cloudinary 업로드
 *
 * 처리:
 * 1. ImageCache에서 folder LIKE '%Images%' AND cloudinaryUrl IS NULL인 다음 29개 조회
 * 2. 각 파일을 로컬 경로에서 읽기 (/public/local-assets/Images/...)
 * 3. Cloudinary에 업로드 (folder: "cruise-images/기항지/...")
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!cronSecret || authHeader !== `Bearer ${cronSecret}`) {
      logger.warn('[UploadImagesBatch2] 인증 실패 - 무단 접근 시도');
    ...
    logger.log('[UploadImagesBatch2] 배치 2/2 시작', {
    ...
    logger.log('[UploadImagesBatch2] 업로드 대상 조회 완료', {
      totalImages: imagesToSync.length,
      folders: uniqueFolders,
    });

    if (imagesToSync.length === 0) {
      logger.log('[UploadImagesBatch2] 업로드할 이미지 없음 - 작업 종료');
      return NextResponse.json({
        ok: true,
        uploaded: 0,
        failed: 0,
        batchNumber: '2/2',
        message: '업로드할 이미지 없음 (모두 완료됨)',
      });
    }
    ...
    for (let batchIndex = 0; batchIndex < batches; batchIndex++) {
      ...
      logger.log(`[UploadImagesBatch2] 배치 2/2: ${end}/${imagesToSync.length} (${progressPercent}%) 업로드 중...`);
      ...
      const results = await Promise.allSettled(
        batch.map(async (image) => {
          try {
            ...
            // Cloudinary 폴더 경로: cruise-images/기항지/{폴더명}
            ...
            logger.log('[UploadImagesBatch2] 성공:', {
              imageId: image.id,
              fileName: image.fileName,
            ...
            logger.error('[UploadImagesBatch2] 실패:', {
              fileName: image.fileName,
              folder: image.folder,
              error: errorMessage,
            });
    ...
    // 배치 2/2 완료 로그
    logger.log('[UploadImagesBatch2] 배치 2/2 완료', {
      batchNumber: '2/2',
    ...
    return NextResponse.json({
      ...
      successRate: `${successRate}%`,
      elapsedTime: `${elapsedTime}초`,
      message: `배치 2/2 완료: ${uploaded}/${imagesToSync.length} 업로드, ${failed}개 실패`,
    ...
    logger.error('[UploadImagesBatch2] 예상치 못한 에러', {
      error: errorMessage,
      elapsedTime: `${elapsedTime}초`,

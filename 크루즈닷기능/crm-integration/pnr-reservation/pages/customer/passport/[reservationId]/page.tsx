...
export default function CustomerPassportPage() {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          // 파일 크기에 따라 압축률 조절
          // - 10MB 이상: 1200px, 품질 0.6
          // - 5MB 이상: 1400px, 품질 0.7
          // - 5MB 미만: 1600px, 품질 0.8
          let MAX_SIZE = 1600;
          let quality = 0.8;
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, travelerIndex: number) => {
    ...
    // 20MB까지 허용 (클라이언트에서 자동 압축하므로 큰 파일도 OK)
    ...
    try {
      ...
      if (data.ok && data.data) {
        ...
        // travelerId가 있으면 전달하여 Traveler.passportImage 필드도 업데이트
        const travelerId = updatedTravelers[travelerIndex].id;
        const uploadUrl = travelerId
          ? `/api/customer/passport-upload?reservationId=${reservationId}&travelerId=${travelerId}`
          : `/api/customer/passport-upload?reservationId=${reservationId}`;

...
export default function ManualPassportRequestPage() {
  ...
  const loadRequests = useCallback(async () => {
    try {
      ...
      if (!res.ok || !json.ok) {
        throw new Error('여권 요청 목록을 불러오지 못했습니다.');
      }

      // PassportRequestCustomer를 PassportRequest 형식으로 변환
      ...
      logger.error('[ManualPassportRequest] load error', { error: error?.message });
      showError('여권 요청 목록을 불러오는 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  }, [searchQuery]);
  ...
  const loadTemplates = useCallback(async () => {
    try {
      ...
      logger.error('[ManualPassportRequest] template load error', { error: error?.message });
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      ...
      logger.error('[ManualPassportRequest] manual error', { error: error?.message });
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-100 pb-24">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 pt-10 md:px-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-extrabold text-gray-900">수동 여권 요청</h1>
        </div>

#!/usr/bin/env node
// 로컬 빠른 동기화: Google Drive → Cloudinary
...
const CONCURRENT = 15; // 동시 업로드 수 (로컬이므로 여유있게)
const BATCH_LOG_INTERVAL = 50; // 50개마다 진행률 표시
...
async function main() {
  console.log('\n🚀 Google Drive → Cloudinary 로컬 빠른 동기화\n');
  ...
  console.log(`\n📊 동기화 대상: ${pendingImages.length.toLocaleString()}개 이미지\n`);
  ...
  for (let i = 0; i < pendingImages.length; i += CONCURRENT) {
    ...
    // 진행률 표시
    const completed = i + CONCURRENT;
    const progressPercent = Math.round((completed / pendingImages.length) * 100);

    if ((i + CONCURRENT) % BATCH_LOG_INTERVAL === 0 || completed >= pendingImages.length) {
      ...
      console.log(
        `  📈 ${completed.toLocaleString()}/${pendingImages.length} (${progressPercent}%) | ` +

...
async function uploadToDriveAndCloudinary(
  ...
  try {
    ...
    try {
      ...
      logger.info('[Batch2Sync] Upload successful:', {
        imageId: image.id,
        fileName: image.fileName,
...
async function executeSyncOperation(): Promise<BatchSyncResponse> {
  ...
  try {
    logger.info(`[Batch2Sync] Starting Cloudinary upload for ${BATCH_LABEL}`);
    ...
    logger.info(`[Batch2Sync] Upload target: ${totalImages.toLocaleString()} images`);

    if (totalImages === 0) {
      logger.info('[Batch2Sync] All images in Images folder already synced');
    ...
    for (let i = 0; i < totalImages; i += CONCURRENT) {
      ...
      logger.info(`[Batch2Sync] ${progressMsg}`);
    }
    ...
    logger.info('[Batch2Sync] Batch sync completed', {
      batchLabel: BATCH_LABEL,
      totalImages,
      successCount,
      failureCount,

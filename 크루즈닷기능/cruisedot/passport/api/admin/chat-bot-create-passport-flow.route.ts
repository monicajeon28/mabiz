  ...
  try {
    ...
    if (existingFlow) {
      return NextResponse.json({
        ...
        shareToken: existingFlow.shareToken,
      });
    }
    ...
    const questions = [
      {
        ...
        nextQuestionIdA: null, // 다음 질문은 동적으로 결정
      },
      {
        flowId: flow.id,
        order: 2,
        ...
        nextQuestionIdA: null, // 2인 1실 선택 시
        nextQuestionIdB: null, // 3인 이상 선택 시
      },
      {
        flowId: flow.id,
        order: 3,
    ...
    if (createdQuestions.length >= 5) {
      ...
      await prisma.chatBotQuestion.update({
        ...
        data: {
          nextQuestionIdA: createdQuestions[2].id, // 2인 1실
          nextQuestionIdB: createdQuestions[3].id, // 3인 이상
        },
      });
    ...
    console.error('[Create Passport Flow] Error:', error);
    return NextResponse.json(

...
export async function POST(req: NextRequest) {
  try {
    ...
    // 이미지 다운로드
    logger.log('[OCR to APIS] 이미지 다운로드 시작:', body.imageUrl);
    ...
    try {
      ...
      logger.error('[OCR to APIS] 이미지 다운로드 오류:', error);
      return NextResponse.json(
...
Return ONLY the JSON object now:`;
    ...
    try {
      ...
      logger.error('[OCR to APIS] OCR 처리 오류:', ocrError);
      return NextResponse.json(
    ...
    if (!submission.UserTrip || !submission.UserTrip.CruiseProduct) {
      return NextResponse.json(
    ...
    const productCode = submission.UserTrip.CruiseProduct.productCode;
    ...
    try {
      ...
      // 1. 마스터 시트 업데이트
      await enqueueApisSync('MASTER_SHEET', body.userId);

      // 2. 여행별 시트 업데이트
      await enqueueApisSync('TRIP_SHEET', trip.id);
      ...
      logger.error('[OCR to APIS] DB 저장 오류:', dbError);
      return NextResponse.json(
    ...
    logger.error('[OCR to APIS] POST error:', error);
    return NextResponse.json(
      {
        ok: false,

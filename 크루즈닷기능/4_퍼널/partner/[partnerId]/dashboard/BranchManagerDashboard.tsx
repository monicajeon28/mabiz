...
import { useInputModal } from '@/components/ui/InputModalProvider';
...
interface PartnerProfile {
  ...
  coverImage?: string | { url?: string; id?: string } | null;
...
interface SelectedContractWithMeta {
  id: number;
  name: string;
  phone: string;
  email: string | null;
  ...
  metadata?: {
    signatures?: {
      education?: { url?: string; originalName?: string };
      b2b?: { url?: string; originalName?: string };
    };
    signature?: string;
    [key: string]: unknown;
  } | null;
...
interface PurchasedReservationDetail {
  id: number;
  totalPeople: number;
  pnrStatus: string;
  user: {
...
export default function BranchManagerDashboard({ user, profile, trialInfo }: PartnerDashboardProps) {
  ...
  const [selectedContract, setSelectedContract] = useState<SelectedContractWithMeta | null>(null);
  ...
  const [selectedPurchasedReservation, setSelectedPurchasedReservation] = useState<(typeof purchasedReservations)[number] | null>(null);
  ...
  const [purchasedReservationDetail, setPurchasedReservationDetail] = useState<PurchasedReservationDetail | null>(null);
  ...
  // 결제 시작 함수 — ref로 관리해 countdown interval의 stale closure 방지
  const handleInitiatePaymentRef = useRef<() => void>(() => {});
  const handleInitiatePayment = useCallback(() => {
    ...
    setPendingPaymentAction(async () => {
      try {
        const res = await csrfFetch('/api/partner/subscription/payment', {
          method: 'POST',
          headers: {
  ...
  // handleInitiatePayment ref 동기화 — paymentForm이 바뀔 때마다 최신 함수로 갱신
  useEffect(() => {
    handleInitiatePaymentRef.current = handleInitiatePayment;
      ...
      if (diffMs <= 0) {
        ...
        if (subscriptionInfo.isTrial && subscriptionInfo.status === 'trial') {
          ...
          if (!showPaymentConfirmModal && !showFeatureRestrictionModal) {
            handleInitiatePaymentRef.current();
          }
        }
        return;
      }
  ...
  }, [subscriptionInfo, showPaymentConfirmModal, showFeatureRestrictionModal]);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (json.ok && json.contractTypes) {
          ...
          const filteredTypes = json.contractTypes.filter((ct: { code: string }) => ct.code !== 'AFFILIATE');
        ...
        if (res.ok) {
          ...
          if (json.ok && json.links) {
            ...
            const commonLinks = json.links
              .filter((link: { isCommonLink: boolean; productCode: string | null; status: string }) => link.isCommonLink && link.productCode && link.status === 'ACTIVE')
              .map((link: { id: number; code: string; title: string | null; productCode: string | null; url: string; product: { id: number; productCode: string; title: string } | null }) => ({
                id: link.id,
                code: link.code,
                title: link.title,
  ...
  // 이미지 URL 추출 헬퍼 함수
  const extractImageUrl = (image: string | { url?: string; id?: string } | null | undefined): string | null => {
  ...
  const loadMySales = async (month?: string) => {
    try {
      ...
      showError((error instanceof Error ? error.message : String(error)) || '판매 목록을 불러오는 중 오류가 발생했습니다');
    } finally {
    ...
    try {
      ...
      if (res.ok && json.ok) {
        ...
        const teamOnlySales = (json.sales || [])
          .filter((sale: { agentId: number | null }) => sale.agentId && sale.agentId !== json.profileId)
          .map((sale: {
            id: number;
            productCode: string | null;
            ...
            agent?: { displayName?: string | null; User?: { name?: string | null } };
            AffiliateLead?: { customerName?: string | null };
          }) => ({
            id: sale.id,
            ...
            agentName: sale.agent?.displayName ||
                       sale.agent?.User?.name || '알 수 없음',
  ...
  type PurchasedReservationItem = (typeof purchasedReservations)[number];

  // 구매고객관리 핸들러 함수들
  const handleOpenPassportModal = (reservation: PurchasedReservationItem) => {
  ...
  const handleSendPassportMessage = async () => {
    ...
    try {
      ...
      const response = await csrfFetch('/api/partner/customers/send-sms', {
        method: 'POST',
      ...
      showError((error instanceof Error ? error.message : String(error)) || '문자 발송 중 오류가 발생했습니다.');
    } finally {
  ...
  const handleOpenChatbotModal = async (reservation: PurchasedReservationItem) => {
    setSelectedPurchasedReservation(reservation);
    ...
    try {
      ...
      showError((error instanceof Error ? error.message : String(error)) || '챗봇 링크를 생성하는 중 오류가 발생했습니다.');
    }
  };
  ...
  const handleOpenPurchasedDetailModal = async (reservation: PurchasedReservationItem) => {
    try {
  ...
  const handleCompleteContract = async (contractId: number) => {
    ...
    if (!(await confirm({ message: '이 계약서를 완료하여 PDF를 이메일로 전송하시겠습니까?', isDangerous: true }))) return;

    try {
      setCompletingContractId(contractId);
      const res = await csrfFetch(`/api/partner/contracts/${contractId}/complete`, {
        method: 'POST',
        credentials: 'include',
      });
      ...
      showError((error instanceof Error ? error.message : String(error)) || '계약서 완료 처리 중 오류가 발생했습니다.');
    } finally {
  ...
  const handleSendPdf = async (contractId: number) => {
    if (!(await confirm({ message: '계약서 PDF를 계약자 이메일 주소로 전송하시겠습니까? (본사 이메일은 참조로 추가됩니다)', isDangerous: true }))) return;
    try {
      ...
      try {
        const res = await csrfFetch(`/api/partner/contracts/${contractId}/send-pdf`, {
          method: 'POST',
          credentials: 'include',
        ...
        clearTimeout(timeoutId);
        if ((fetchError instanceof Error ? fetchError.name : 'Error') === 'AbortError') {
      ...
      showError((error instanceof Error ? error.message : String(error)) || 'PDF 전송 중 오류가 발생했습니다.');
    } finally {
  ...
  const handleReject = async (contractId: number) => {
    const reason = await inputModal({ title: '계약 거부', label: '거부 사유', placeholder: '사유를 입력하세요', isDangerous: true });
    if (!reason) return;

    try {
      const res = await csrfFetch(`/api/partner/contracts/${contractId}/reject`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      showError((error instanceof Error ? error.message : String(error)) || '계약 거부 중 오류가 발생했습니다.');
    }
  };

  const handleDelete = async (contractId: number) => {
    if (!(await confirm({ message: '정말로 이 계약서를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.', isDangerous: true }))) return;
    try {
      setDeletingContractId(contractId);
      const res = await csrfFetch(`/api/partner/contracts/${contractId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const json = await res.json();
      ...
      showError((error instanceof Error ? error.message : String(error)) || '삭제 중 오류가 발생했습니다.');
    } finally {
  ...
  const handleViewDetail = async (contractId: number) => {
    try {
      ...
      showError((error instanceof Error ? error.message : String(error)) || '계약서 정보를 불러오는 중 오류가 발생했습니다.');
    } finally {
  ...
  const handleRegisterCustomer = async () => {
    ...
    try {
      const response = await csrfFetch('/api/affiliate/customers/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const copyB2BLink = async () => {
    if (!b2bLandingUrl) return;
    try {
      await navigator.clipboard.writeText(b2bLandingUrl);
  ...
  const copyTrialDashboardLink = async () => {
    if (!trialDashboardUrl) return;
    try {
      await navigator.clipboard.writeText(trialDashboardUrl);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      {subscriptionInfo && countdown && (
        <div className={`fixed top-0 left-0 right-0 z-50 ${subscriptionBannerDismissed ? 'hidden' : ''} ${subscriptionInfo.isTrial
          ? 'bg-gradient-to-r from-slate-800 to-slate-900'
          : subscriptionInfo.status === 'active'
            ? 'bg-gradient-to-r from-slate-900 to-slate-800'
            : 'bg-gradient-to-r from-slate-700 to-slate-800'
          } text-white shadow-lg shadow-slate-900/10`}>
          <div className="mx-auto max-w-7xl px-4 py-2.5 md:py-3 relative">
            <button
            ...
            <div className="flex items-center justify-center gap-3 md:gap-4 flex-wrap pr-8">
      ...
      <div className={`mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 md:gap-8 md:px-6 ${subscriptionInfo ? (isInstallable || (!isStandalone && /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(typeof navigator !== 'undefined' ? navigator.userAgent : ''))) && !isStandalone ? 'pt-32 md:pt-36' : 'pt-20 md:pt-24' : 'pt-6 md:pt-10'
        ...
        {/* ── 고객 공유 링크 (최우선 CTA) ─────────────────────────────────────── */}
        {user.mallUserId && (
          <div className="rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 p-5 shadow-lg shadow-blue-500/20 mb-6">
            <div className="flex items-center gap-2 mb-1">
              <FiLink className="text-white text-lg" />
              <p className="text-sm font-bold text-white">고객에게 공유할 내 판매몰</p>
            </div>
            <p className="text-xs text-blue-100 mb-4">
              이 링크 하나로 어떤 상품이든 구매 시 자동으로 내 실적에 연결됩니다 · 카카오톡·문자·SNS로 바로 공유하세요
            </p>
            {mallFullUrl ? (
              <div className="flex items-center gap-2">
                <div className="flex-1 min-w-0 rounded-xl bg-white/15 px-3 py-2.5 backdrop-blur-sm">
                  <a
                    href={`/${user.mallUserId}/shop`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block truncate text-xs font-semibold text-white font-mono hover:underline"
                  >
                    {mallFullUrl}
                  </a>
                </div>
                <button
                  onClick={async () => {
                    try {
                      await navigator.clipboard.writeText(mallFullUrl);
                      showSuccess('판매몰 링크가 복사되었습니다!');
                    } catch {
                      showError('링크 복사에 실패했습니다.');
                    }
                  }}
                  className="flex-shrink-0 rounded-xl bg-white px-4 py-2.5 text-xs font-bold text-blue-700 shadow-sm hover:bg-blue-50 active:scale-95 transition-all"
                >
                  복사
                </button>
              </div>
            ) : (
              <div className="h-10 animate-pulse rounded-xl bg-white/15" />
            )}
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-lg shadow-slate-900/5 border border-slate-100 p-4 md:p-6 mb-6">
          ...
          {stats?.selectedMonth && (
            <p className="mt-3 text-xs text-slate-500 bg-slate-50 rounded-lg px-3 py-2">
              선택된 월: {(() => {
                const parts = stats.selectedMonth.split('-');
                const year = parts[0] || '';
        ...
        {/* 크루즈 렌탈 콜 스크립트 — 대리점장 편집 가능 */}
        <CallScriptSection canEdit={true} />
        ...
        <section className="rounded-2xl bg-white p-4 shadow-lg shadow-slate-900/5 border border-slate-100 md:rounded-3xl md:p-6">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
            ...
            {/* 판매원 간편 가입 링크 공유 */}
            {isBranchManager && profile?.affiliateCode && (
              <button
                onClick={async () => {
                  const joinUrl = `${window.location.origin}/affiliate/join?ref=${profile.affiliateCode}`;
                  try {
                    await navigator.clipboard.writeText(joinUrl);
                    showSuccess('판매원 초대 링크가 복사되었습니다.');
                  } catch {
                    showError('링크 복사에 실패했습니다.');
                  }
                }}
                className="flex flex-col items-center justify-center gap-2 rounded-xl bg-blue-50 p-4 text-center transition-all hover:bg-blue-100 hover:shadow-lg md:p-6"
              >
                <FiLink className="text-2xl text-blue-600 md:text-3xl" />
                <span className="text-xs font-semibold text-blue-700 md:text-sm">판매원 초대</span>
              </button>
            )}
        ...
        {stats && (
          <div className="grid gap-6 md:grid-cols-2">
            ...
            {/* 구매 문의 관리 — 내 링크로 들어온 문의 */}
            <div className="rounded-2xl bg-white p-4 shadow-lg shadow-slate-900/5 border border-slate-100 md:rounded-3xl md:p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-slate-900 md:text-xl">구매 문의 관리</h2>
                <Link
                  href={`${partnerBase}/inquiries`}
                  className="text-xs text-slate-600 hover:text-slate-900 font-medium md:text-sm"
                >
                  전체보기 <FiArrowRight className="inline ml-1" />
                </Link>
              ...
              <p className="text-sm text-gray-500 mb-4">
                내 링크를 통해 들어온 구매 문의 고객을 확인하고 콜 기록을 남길 수 있습니다.
              </p>
              <div className="flex gap-2 flex-wrap">
                {[
                  { status: 'pending',    label: '대기중',   color: 'bg-yellow-100 text-yellow-800' },
                  { status: 'unavailable', label: '부재중',  color: 'bg-orange-100 text-orange-800' },
                  { status: 'confirmed',  label: '구매확정', color: 'bg-green-100 text-green-800' },
                ].map(item => (
                  <Link
                    key={item.status}
                    href={`${partnerBase}/inquiries?status=${item.status}`}
                    className={`px-3 py-1.5 rounded-xl text-xs font-semibold ${item.color} hover:opacity-80 transition-opacity`}
                  >
                    {item.label}
                  </Link>
                ))}
              ...
              <Link
                href={`${partnerBase}/inquiries`}
                className="mt-4 flex items-center justify-center gap-2 w-full py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-sm font-semibold transition-colors"
              >
                📞 콜 기록 & 문의 관리
              </Link>
            </div>
                            ...
                            <div className="flex-1">
                              ...
                              {(agent.phone || agent.email) && (
                                <div className="mt-1 space-y-0.5">
                                  ...
                                  {agent.email && (
                                    <p className="text-xs text-gray-500">✉️ {agent.email}</p>
                                  )}
                                </div>
                              )}
                              {agent.lastSaleAt && (
                                <p className="text-xs text-gray-400 mt-0.5">
                                  마지막 활동: {new Date(agent.lastSaleAt).toLocaleDateString('ko-KR')}
                                </p>
                              )}
                              {!agent.lastSaleAt && (
                                <p className="text-xs text-red-400 mt-0.5">판매 기록 없음</p>
                              )}
                            </div>
            ...
            {/* 담당 멘토 정보 (판매원만) */}
            {isSalesAgent && profile.agentRelations?.[0]?.manager && (
              <section className="rounded-2xl bg-white p-4 shadow-lg md:rounded-3xl md:p-6">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-lg font-bold text-slate-900 md:text-xl flex items-center gap-2">
                ...
                {(() => {
                  const mentor = profile.agentRelations[0].manager;
                  return (
                    <div className="space-y-3">
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-lg border border-gray-200 bg-white p-4">
                    ...
                    {purchasedReservationDetail.travelers && purchasedReservationDetail.travelers.length > 0 ? (
                      <div className="space-y-4">
                        {purchasedReservationDetail.travelers.map((traveler, index: number) => (
                          <div key={traveler.id || index} className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                            ...
                            <div className="grid gap-3 md:grid-cols-2">
                              ...
                              <div>
                                ...
                                {traveler.hasPassportImage && (
                                  <p className="text-xs text-green-600 mt-1">✓ 여권 이미지 제출됨</p>
                                )}
                              </div>
                              <div>
                                <p className="text-sm font-medium text-gray-500">생년월일</p>
                                <p className="text-base text-gray-900">{traveler.birthDate || '미입력'}</p>
                              </div>
                              <div>
                                <p className="text-sm font-medium text-gray-500">여권 만료일</p>
                                <p className="text-base text-gray-900">{traveler.expiryDate || '미입력'}</p>
                              </div>
                              <div>

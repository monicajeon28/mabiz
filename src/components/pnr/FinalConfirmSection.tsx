...
export default function FinalConfirmSection({ leadId, customerName, onStatusChange }: Props) {
  ...
  const getStatusBadge = () => {
    switch (status) {
      case 'APPROVED':
        return (
          <span className="inline-flex items-center gap-1 rounded-full bg-green-100 px-3 py-1.5 text-sm font-bold text-green-700 border border-green-200">
            <CheckCircle className="h-3 w-3" /> 승인완료
          </span>
        );
      case 'REQUESTED':
        return (
          <span className="inline-flex items-center gap-1 rounded-full bg-orange-100 px-3 py-1.5 text-sm font-bold text-orange-700 border border-orange-200">
            <Clock className="h-3 w-3" /> 본사 확인 대기중
          </span>
        );
      case 'REJECTED':
        return (
          <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-3 py-1.5 text-sm font-bold text-red-700 border border-red-200">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 text-sm font-bold text-slate-600 border border-slate-200">
            <Clock className="h-3 w-3" /> 요청 가능
          </span>
        );
    }
  };
      ...
      {status === 'REQUESTED' && (
        <div className="space-y-3">
          ...
          {confirmStatus?.audioUrl && (
            ...
            >
              <Mic className="h-4 w-4" /> 제출한 콜녹음 듣기 <ExternalLink className="h-3 w-3" />
            </a>
          )}
        </div>
      )}
      ...
      {status === 'APPROVED' && (
        <div className="space-y-3">
          ...
          {confirmStatus?.audioUrl && (
            ...
            >
              <Mic className="h-4 w-4" /> 콜녹음 듣기 <ExternalLink className="h-3 w-3" />
            </a>
          )}
        </div>
      )}

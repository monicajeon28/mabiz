...
export function ContactGroupsPanel() {
  ...
  useEffect(() => {
    ...
    fetch("/api/csrf-token", { signal: ctrl.signal })
      ...
      .catch((err) => { if (err.name !== 'AbortError') logger.error("[ContactGroupsPanel] csrf", { err }); });
    return () => ctrl.abort();
  }, []);
  ...
  const loadGroups = useCallback(async () => {
    ...
    try {
      ...
      logger.error("[ContactGroupsPanel] loadGroups", { err });
  ...
  const deleteSelected = async () => {
    ...
    try {
      ...
      logger.error("[ContactGroupsPanel] deleteSelected", { err });
      ...
      <div className="flex flex-col h-full space-y-3">
        ...
        <div>
          <p className="text-base text-gray-600 leading-relaxed">
            랜딩페이지·자동문자에 연결되는 연락처 그룹입니다. 크루즈닷 회원 분류는 옆의{" "}
            <span className="font-bold text-blue-700">회원 그룹</span> 탭에서 관리합니다.
            두 그룹은 서로 분리되어 있습니다.
          </p>
        </div>
        {/* ═══ 상단 버튼바 1행 ════════════════════════════════════════════ */}

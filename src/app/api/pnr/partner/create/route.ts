...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ⚠️ CruiseProduct 존재 확인 (Prisma에 CruiseProduct 모델이 없으므로 raw query 사용)
    // tripId는 실제로 CruiseProduct.id를 의미함
    const cruiseProducts = await prisma.$queryRaw<Array<{
      id: number;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!mainUserData) {
        ...
        mainUserData = await tx.gmUser.create({
          data: {
        ...
        if (Object.keys(updateData).length > 0) {
          mainUserData = await tx.gmUser.update({
      ...
      for (let i = 0; i < travelers.length; i++) {
        ...
        if (isSingleCharge && traveler.passportNo && passportToUserIdMap.has(traveler.passportNo)) {
          ...
          logger.log(`[Reservation Create] 싱글차지: 동일 여권번호(${traveler.passportNo})를 가진 여행자에 기존 User(${existingUserId}) 재사용`);
          continue;
        }

        // 우선순위 1: phone 번호로 찾기
        if (traveler.phone) {
          travelerUser = await tx.gmUser.findFirst({
        ...
        if (!travelerUser) {
          ...
          travelerUser = await tx.gmUser.create({
            data: {
          ...
          if (Object.keys(updateData).length > 0) {
            travelerUser = await tx.gmUser.update({
              where: { id: travelerUser.id },
              data: updateData,
            });
            logger.log(`[Reservation Create] 기존 User 정보 업데이트 (찾은 방법: ${foundBy}): userId=${travelerUser.id}`);
          }
        }
      ...
      if (isSingleCharge) {
        ...
        logger.log(`[Reservation Create] 싱글차지 처리: totalPeople=${totalPeople}, 동일 여권번호 중복 입력 허용`);
      }
      ...
      const reservation = await tx.gmReservation.create({
        data: {
      ...
      for (let i = 0; i < travelers.length; i++) {
        ...
        const createdTraveler = await tx.gmTraveler.create({
          data: travelerData,
        });

        logger.log(`[Reservation Create] Traveler 생성 완료 (APIS 호환): travelerId=${createdTraveler.id}, korName=${createdTraveler.korName}`);
    ...
    logger.log(`[Reservation Create] 예약 생성 완료: reservationId=${result.reservation.id}`);
      ...
      for (let i = 0; i < result.travelers.length; i++) {
        ...
        if (originalTraveler?.passportImage && createdTraveler.passportNo) {
          try {
            ...
            // 구글 드라이브 백업 (google-drive 모듈의 getDriveClient + findOrCreateFolder 사용)
            ...
            if (trip) {
              // CruiseProduct 정보를 raw query로 조회
              const tripProducts = trip.productId
                ? await prisma.$queryRaw<Array<{ shipName: string | null }>>`SELECT "shipName" FROM "CruiseProduct" WHERE id = ${trip.productId}`
                : [];
              const tripProduct = tripProducts[0] ?? null;
              const shipName = tripProduct?.shipName || trip.shipName || 'UNKNOWN';
              ...
              const tripFolderId = await findOrCreate(tripFolderName, rootFolderId);
              ...
              const groupFolderId = await findOrCreate(groupFolderName, tripFolderId);
              ...
              const uploadResult = await drive.files.create({
                requestBody: { name: fileName, parents: [groupFolderId] },
              ...
              if (uploadResult.data.id) {
                logger.log(`[Reservation Create] 여권 이미지 구글 드라이브 백업 완료: https://drive.google.com/file/d/${uploadResult.data.id}/view`);
              }
            }
          } catch (error) {
            logger.error('[Reservation Create] 여권 이미지 구글 드라이브 백업 실패:', error as Record<string, unknown>);
            // 백업 실패해도 예약은 계속 진행
          }
        }
      }
    ...
    // 고객 여정 기록 (customer-journey 모듈 미구현 - 추후 연동 예정)
    // TODO: recordCustomerJourney 함수 구현 후 활성화
    ...
    if (apisSyncResult && !apisSyncResult.ok) {
      logger.error(`[Reservation Create] APIS 자동화 최종 실패 (${apisSyncResult.retryCount}회 시도):`, { error: apisSyncResult.error });
      logger.error('[Reservation Create] 관리자가 수동으로 APIS를 입력해야 합니다.');
    }

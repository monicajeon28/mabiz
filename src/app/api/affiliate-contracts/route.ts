...
export async function GET(req: NextRequest) {
  try {
    ...
    const contracts = rows.map((r) => ({
      ...
      type:             r.type,
      status:           r.status,
      startDate:        r.startDate ? new Date(r.startDate).toISOString() : null  ?? null,
      endDate:          r.endDate ? new Date(r.endDate).toISOString() : null    ?? null,
      signedAt:         r.signedAt ? new Date(r.signedAt).toISOString() : null   ?? null,
      note:             r.note,

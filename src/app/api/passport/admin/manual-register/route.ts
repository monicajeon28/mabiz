...
const GuestSchema = z.object({
  ...
  passportNumber: z
    .string()
    .regex(/^[A-Z0-9]{6,9}$/, '올바른 여권 번호 형식이 아닙니다')
    .optional()
    .or(z.literal('')),
...
const ManualRegisterSchema = z.object({
  ...
  groups: z
    .array(GroupSchema)
    .min(1, '최소 1개의 그룹이 필요합니다')
    .max(30, '최대 30개 그룹까지 가능합니다'),
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      logger.error('[ManualRegister] JSON parsing error:', error as Record<string, unknown>);
      return NextResponse.json(
    ...
    // User의 최신 여행 정보 조회
    const latestTrip = await prisma.gmTrip.findFirst({
    ...
    if (validGroups.length === 0) {
      return NextResponse.json(
        { ok: false, message: '최소 한 개 이상의 그룹이 필요합니다.' },
        { status: 400 },
      );
    }
      ...
      return group.guests
        .map((guest) => ({
          ...
          passportExpiryDate: guest.passportExpiryDate
            ? new Date(guest.passportExpiryDate)
            : null,
        }))
    ...
    if (guestRecords.length === 0) {
      return NextResponse.json(
        { ok: false, message: '각 그룹에 최소 한 명 이상의 탑승자를 입력해주세요.' },
        { status: 400 },
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const existingSubmission = await tx.gmPassportSubmission.findFirst({
        where: { userId: user.id, isSubmitted: false },
        orderBy: { createdAt: 'desc' },
      });

      if (existingSubmission) {
        await tx.gmPassportSubmissionGuest.deleteMany({
          where: { submissionId: existingSubmission.id },
        });
        await tx.gmPassportSubmission.delete({
      ...
      const submission = await tx.gmPassportSubmission.create({
        data: createData,
      });
      ...
      for (const guest of guestRecords) {
        ...
        if (!isSameAsPurchaser && guest.name && guest.phone) {
          ...
          if (mobilePattern.test(normalizedPhone)) {
            const existingUser = await tx.gmUser.findFirst({
              where: {
                name: guest.name,
      ...
      try {
        await tx.gmPassportRequestLog.create({
          data: {
            userId: user.id,
            adminId: manager.id,
            templateId: null,
            messageBody: `수동 여권 등록 (관리자: ${manager.name})`,
        ...
        logger.warn('[ManualRegister] Failed to insert log:', logError as Record<string, unknown>);
      }
    ...
    return NextResponse.json({ ok: true, data: successResult });
  } catch (error) {
    logger.error('[ManualRegister] POST error:', error as Record<string, unknown>);
    logger.error('[ManualRegister] Error details:', {
      message: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
    });
    return NextResponse.json(
      { ok: false, message: '수동 여권 등록 중 오류가 발생했습니다.' },
      { status: 500 },
    );
  }
}

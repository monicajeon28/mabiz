...
export async function GET(req: NextRequest) {
  try {
    ...
    const sales = rows.map((r) => ({
      ...
      saleDate:           r.saleDate ? new Date(r.saleDate).toISOString() : null    ?? null,
      confirmedAt:        r.confirmedAt ? new Date(r.confirmedAt).toISOString() : null ?? null,
      refundedAt:         r.refundedAt ? new Date(r.refundedAt).toISOString() : null  ?? null,

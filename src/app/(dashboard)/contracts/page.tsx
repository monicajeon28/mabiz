  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
        ...
        >
          ...
          >
            ...
            <div className="space-y-3 mb-6">
              ...
              {/* 옵션 D: 계약서 복제 (COMPLETED 상태에서만 활성화) */}
              {selectedContract.status === "completed" && (
                <button
                  onClick={() => {
                    setDuplicateModal({
                      isOpen: true,
                      contractInstanceId: selectedContract.id,
                    });
                    setShowOptionsModal(false);
                  }}
                  className="w-full p-4 border-2 border-purple-300 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors text-left"
                >
                  <div className="flex items-start gap-3">
                    <span className="text-2xl">📋</span>
                    <div>
                      <p className="font-semibold text-purple-700">옵션 D: 계약서 복제</p>
                      <p className="text-sm text-purple-600 mt-1">같은 템플릿으로 새 계약서 생성</p>
                    </div>
                  </div>
                </button>
              )}
      ...
      <DuplicateContractModal
        contractInstanceId={duplicateModal.contractInstanceId}
        isOpen={duplicateModal.isOpen}
        onClose={() =>
          setDuplicateModal({ isOpen: false, contractInstanceId: '' })
        }
        onSuccess={(newContractInstanceId) => {
          toast({
            title: '성공',
            description: '새로운 계약서로 이동합니다',
            variant: 'success',
          });
          // 새 계약서 페이지로 이동 (또는 목록 새로고침)
          // router.push(`/contracts/${newContractInstanceId}`);
          // 여기서는 목록 새로고침
          setTimeout(() => {
            fetchContracts();
          }, 1000);

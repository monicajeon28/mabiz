...
${footerBlock}
  ...
  const save = async () => {
    ...
    // 결제 ON이면 상품명·금액(100원 이상) 필수 — 안 넣으면 공개페이지 결제하기 버튼이 막힘(PayApp 최소금액)
    if (paymentEnabled) {
            ...
            {commentEnabled && (
              <div className="p-4">
                <p className="text-sm text-blue-700 bg-blue-50 px-3 py-2 rounded-lg leading-relaxed">
                  💡 방문자가 질문하고 서로 답하는 커뮤니티 게시판이에요. <b>저장 후 → 랜딩페이지 관리(편집) → 댓글 탭</b>에서
                  <b>개수·작성 기간</b>을 정하면 AI가 질문·경험·응원 댓글을 그 기간에 날짜를 분산해 자동 생성합니다(처음부터 북적이게).
                  그 위에 진짜 방문자 질문이 쌓여요.
                </p>
              </div>
            )}

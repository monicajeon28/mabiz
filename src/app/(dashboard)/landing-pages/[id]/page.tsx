...
export default function EditLandingPage() {
  ...
  const [seeding,       setSeeding]      = useState(false); // 샘플 문의 자동 생성(큐레이션 Q&A)
  const [commentMsg,    setCommentMsg]   = useState("");
  // 샘플 문의 자동 생성은 관리자(GLOBAL_ADMIN)·지사(OWNER)만 노출 (서버도 동일 권한 강제)
  const { role } = useSession();
  const canSeed = role === "GLOBAL_ADMIN" || role === "OWNER";
  ...
  // 샘플 문의 자동 생성(큐레이션 Q&A 풀, 외부 의존 0) — 관리자/지사 전용. 멱등성: 페이지당 최대 10개.
  const seedComments = async () => {
    setSeeding(true);
    setCommentMsg("");
    try {
      const res = await fetch(`/api/landing-pages/${id}/comments/seed`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ count: genCount, dateFrom: genDateFrom, dateTo: genDateTo }),
      });
      ...
      const data = await res.json();
      if (data.ok) {
        // 생성 결과를 목록 맨 앞에 반영(관리 목록은 isAutoGenerated 등 일부 필드만 사용)
        if (Array.isArray(data.comments) && data.comments.length > 0) {
          setComments((prev) => [
            ...data.comments.map((c: { id: string; authorName: string; content: string; createdAt: string }) => ({
              id: c.id, authorName: c.authorName, content: c.content, isAutoGenerated: true, createdAt: c.createdAt,
            })),
            ...prev,
          ]);
        }
        setCommentMsg(`✅ ${data.message ?? "샘플 문의를 생성했습니다."}`);
      } else {
        setCommentMsg(data.message ?? "생성 실패. 다시 시도하세요.");
      }
    } catch {
      setCommentMsg("생성 중 오류 발생");
    } finally {

...
class SimpleRulesLensDetection {
    ...
    static detect(contact) {
        ...
        // L1: Price objection
        if (contact.notes && contact.notes.toLowerCase().includes("price"))
            score["L1"] += 3;
        if (contact.notes && contact.notes.toLowerCase().includes("비용"))
            score["L1"] += 3;
        ...
        if (contact.notes && contact.notes.toLowerCase().includes("competitor"))
            score["L3"] += 2;
        // L5: Self-projection / medical
        if (contact.source === "MEDICAL" || contact.tags?.includes("medical"))
            score["L5"] += 2;
        // L6: Loss aversion / timing
        if (contact.source === "PROMO" || contact.source === "LIMITED")
            score["L6"] += 2;
        // L7: Companion / family
        if (contact.groupId)
            score["L7"] += 1;
        // L8: Repurchase
        if (contact.totalPurchases && contact.totalPurchases > 1)
            score["L8"] += 2;
        // L10: Immediate purchase
        if (contact.source === "HOTLEAD" || contact.tags?.includes("high_intent"))
            score["L10"] += 3;
        // Find primary lens

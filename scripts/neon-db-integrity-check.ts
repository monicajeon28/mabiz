/**
 * Neon DB 데이터 무결성 검증 스크립트
 *
 * 목표:
 * 1. Neon DB 연결 확인
 * 2. 주요 테이블 데이터 존재 여부 확인
 * 3. FK 참조 무결성 검증
 * 4. 중복 데이터 검사
 * 5. 타임스탬프 이상치 검사
 * 6. 통계 생성 및 보고서 작성
 *
 * 사용:
 *   npx ts-node scripts/neon-db-integrity-check.ts
 */
...
class NeonDBIntegrityChecker {
  ...
  private addIssue(severity: 'ERROR' | 'WARNING' | 'INFO', tableName: string, message: string, count?: number): void {
  ...
  async checkConnection(): Promise<boolean> {
    try {
      ...
      this.addIssue('ERROR', 'connection', `Failed to connect to Neon DB: ${error}`);
      return false;
    }
  }
  ...
  async checkTableCounts(): Promise<void> {
    ...
    for (const table of tablesToCheck) {
      try {
        const count = await this.prisma[table.name as any].count();
  ...
  async checkUsersByRole(): Promise<void> {
    ...
    try {
      ...
      if (Array.isArray(usersByRole)) {
        ...
        for (const row of usersByRole) {
          const r = row as any;
          this.results.statistics.usersByRole[r.role] = r.count;
          this.log(`  - ${r.role}: ${r.count} users`);
        }
      }
    } catch (error) {
      this.addIssue('WARNING', 'User', `Could not analyze user roles: ${error}`);
    }
  }

  async checkProductStats(): Promise<void> {
    ...
    try {
      ...
      this.addIssue('WARNING', 'Product', `Could not check product stats: ${error}`);
    }
  }
  ...
  async checkForeignKeyIntegrity(): Promise<void> {
    ...
    try {
      ...
      const count = (invalidOrgContacts as any[])[0]?.count || 0;
      if (count > 0) {
      ...
      this.addIssue('WARNING', 'Contact', `Could not check FK integrity: ${error}`);
    }
  }

  async checkDuplicateContacts(): Promise<void> {
    ...
    try {
      ...
      if (Array.isArray(duplicates) && duplicates.length > 0) {
        ...
        const dupeCount = (totalDupes as any[])[0]?.count || 0;
      ...
      this.addIssue('WARNING', 'Contact', `Could not check duplicates: ${error}`);
    }
  }

  async checkTimestampAnomalies(): Promise<void> {
    ...
    try {
      ...
      const count = (futureContacts as any[])[0]?.count || 0;
      if (count > 0) {
      ...
      this.addIssue('WARNING', 'Contact', `Could not check timestamps: ${error}`);
    }
  }

  async checkReviewStatistics(): Promise<void> {
    ...
    try {
      ...
      this.addIssue('WARNING', 'ProductReview', `Could not check reviews: ${error}`);
    }
  }
  ...
  async runAllChecks(): Promise<void> {
    try {
      ...
      this.addIssue('ERROR', 'system', `Unexpected error during checks: ${error}`);
    } finally {
